...
if uploaded_file is not None:
    ...
    with tab1:
        ...
        if matched_records:
            ...
            # ================= FULLY FIXED 5th & 6th BOX DIGIT FILTER =================
            st.markdown("---")
            st.subheader("✏️ 5 & 6. चौथे बॉक्स में से अंक (डिजिट) फ़िल्टर:")
            
            # स्थिर key ताकि इनपुट बदलते ही रियल-टाइम रिफ्रेश हो सके
            user_digits_input = st.text_input(
                "🔢 चौथे बॉक्स के नंबरों में से जिन अंकों को अलग करना है, यहाँ लिखें (उदा. 1 3 5 या 2 7):",
                value="1 3 5",
                key="fixed_digit_filter_input"
            )

            # इनपुट स्ट्रिंग से केवल यूनिक अंक (0-9) निकालना
            ...
            if tab3_cut_nums:
                for n in tab3_cut_nums:
                    n_str = f"{n:02d}"
                    # जाँच करें कि क्या नंबर में चुना हुआ कोई भी अंक शामिल है
            ...
            with col_box5:
                ...
                st.text_area(
                    "कॉपी करें (चुने अंक के नंबर):", 
                    value=digit_filter_box_str, 
                    height=180, 
                    key=f"txt_box5_{active_g}_{mode_seq_days_1}_{'_'.join(entered_digits)}"
                )
            
            with col_box6:
                st.markdown(f"📦 **6. इनके अलावा बाकी बचे नंबर (कुल `{remaining_cut_count}` नंबर):**")
                st.text_area(
                    "कॉपी करें (बाकी बचे नंबर):", 
                    value=remaining_cut_box_str, 
                    height=180, 
                    key=f"txt_box6_{active_g}_{mode_seq_days_1}_{'_'.join(entered_digits)}"
                )
